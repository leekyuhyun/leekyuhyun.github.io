      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-6">
        {VALUES_DATA.map((value: ValueItem, index: number) => {
          ...
          return (
            <div
              key={index}
              className="group relative flex flex-col justify-between p-6 md:p-7 rounded-2xl bg-white dark:bg-slate-900/80 border border-slate-200/80 dark:border-slate-800/80 shadow-sm hover:shadow-md hover:border-sky-500/40 dark:hover:border-sky-500/40 transition-all duration-300"
            >
              <div>
                ...
                <h3 className="text-base md:text-lg font-bold text-slate-900 dark:text-slate-100 mb-3 leading-snug group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors">
                  {value.title}
                </h3>
              </div>
              <p className="text-sm md:text-base text-slate-600 dark:text-slate-400 leading-relaxed mt-2">
                {value.description}
              </p>
            </div>
          );
